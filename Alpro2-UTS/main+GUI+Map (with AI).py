...
class TaxiAppGUI:
    def __init__(self, root):
        ...
        ttk.Button(random_frame, text="Randomize All", 
                 command=self.randomize_all).pack(side=tk.LEFT, padx=2)
        ...
        self.randomize_btn = ttk.Button(button_frame, text="Randomize Map", 
                                      command=self.randomize_map_layout)
        self.randomize_btn.pack(side=tk.LEFT, padx=5)
        
        # Efficiency analysis button
        self.efficiency_btn = ttk.Button(button_frame, text="Analyze Efficiency", 
                                       command=self.show_driver_efficiency_analysis)
    ...
    def randomize_map_layout(self):
        """Regenerate the map with new random coordinates"""
        global MAP_COORDINATES
        MAP_COORDINATES = generate_map_coordinates(Local_Map)
        self.draw_map()
        messagebox.showinfo("Map Randomized", "The map layout has been regenerated with new random positions.")
        ...
        try:
            ...
            for driver in Local_Driver_List:
                if driver.get('Name') == selected_driver_name:
                    # Add the total distance to the driver's work minutes (assuming 1 unit = 1 minute)
                    current_work_minutes = int(driver.get('Work_Minutes', 0))
                    new_work_minutes = current_work_minutes + total_dist
                    driver['Work_Minutes'] = str(int(new_work_minutes))
                    
                    # Update driver's location to destination
                    driver['Location'] = destination_location
                    
                    # If work minutes exceed 8 hours (480 minutes), set driver as unavailable
